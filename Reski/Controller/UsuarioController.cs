...

{
    ...
    public class UsuarioController : ControllerBase
    {
        ...
        private UsuarioResponse WithLinks(UsuarioResponse r)
        {
            ...
            var self   = Url.Link(nameof(GetUsuario),    new { id = r.Id, version });
            var update = Url.Link(nameof(UpdateUsuario), new { id = r.Id, version });
            var delete = Url.Link(nameof(DeleteUsuario), new { id = r.Id, version });
        ...
        public async Task<ActionResult<UsuarioResponse>> CreateUsuario([FromBody] UsuarioRequest request)
        {
            try
            {
                ...
                var version = HttpContext.GetRequestedApiVersion()?.ToString() ?? "1.0";
                return CreatedAtAction(nameof(GetUsuario), new { id = usuario.Id, version }, resp);
            }
